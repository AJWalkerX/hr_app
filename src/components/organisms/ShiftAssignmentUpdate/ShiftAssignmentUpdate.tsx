...
import { Button, FormControl, InputLabel, Select, MenuItem, Modal, Box, TextField } from '@mui/material';
...
function ShiftAssignmentUpdate() {
  ...
  const [shiftToDelete, setShiftToDelete] = useState<{ employeeId: number, index: number } | null>(null);
    ...
    if (shiftToDelete) {
      ...
      const updatedEmployees = shiftData.map(employee => {
        if (employee.id === employeeId) {
          const updatedShifts = employee.shifts.filter((_, idx) => idx !== index); // Silme işlemi
  ...
  return (
    <div>
      ...
      {filteredEmployees.map(employee => (
        <div key={employee.id} style={{ marginBottom: '20px', padding: '10px', border: '1px solid #ccc' }}>
          <h2>{employee.name}</h2>
          <img src={employee.avatarUrl} alt={`${employee.name}'s avatar`} width="50" height="50" />
          <p>Email: {employee.email}</p>
          <p>Position: {employee.position}</p>
          <h3>Vardiyalar:</h3>
          <ul>
            {employee.shifts
              .filter(filterShiftsByDate)
              .map((shift, index) => (
                <li key={index}>
                  {formatDate(shift.startDate)} - {formatDate(shift.endDate)}: {shift.shift}
                  <Button
                    variant="outlined"
                    color="primary"
      ...
      <Modal open={openModal} onClose={() => setOpenModal(false)}>
        ...
        >
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '20px' }}>
            ...
            <Button variant="contained" color="secondary" onClick={handleShiftDelete}>
              Evet, Sil
            </Button>
